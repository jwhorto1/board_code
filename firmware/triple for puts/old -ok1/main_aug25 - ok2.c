...
int main(void)
{
 ...
 char   tens_char;
  ...
  Delayms(1000);
    ...
    putsWiFly(OutString);
  Delayms(1000);
    sprintf(OutString,"set ftp timer 8\r\n");
    putsConsole(OutString);
    putsWiFly(OutString);
  Delayms(1000);
...
//    putsWiFly(OutString);
  Delayms(200);
    sprintf(OutString, "Aug25 10:07PM   \nputs only       ");  // columns 41, 59
    putsConsole(OutString);
  Delayms(200);
...
//    putsConsole(OutString);
  Delayms(5000);
    sprintf(AddrString,"ftp put aaah25_00.txt\r\n");
    ones_char = (iloop  % 10);
  ...
  Delayms(5000);
    sprintf(ConstString,"the once and xxth future king]r\n");
