...
// ***************************************************************************
// TO CHANGE THE MESSAGE, put 74.208.7.3 into Browser
// enter as waterman1.symp1e.com, my usual password (without the 'a')
// This sets you up at set comm remote GET$/users/112
//
// ***************************************************************************
...
if (Buttons == 0x04)
 ...
 //   putsLCD(OutString);    }
  ...
  Delayms(5000);
    ...
    sprintf(OutString,"set dns name waterall.herokuapp.com\r\n");
    WiFly_and_Console_out(OutString);
    ...
    WiFly_and_Console_out(OutString);
    sprintf(OutString,"set ip address 192.168.1.115\r\n");
    WiFly_and_Console_out(OutString);
    //sprintf(OutString,"set ip host 107.22.196.175\r\n");
    //
    ...
    WiFly_and_Console_out(OutString);
    sprintf(OutString,"set dns name 68.105.28.12\r\n");
    WiFly_and_Console_out(OutString);
    sprintf(OutString,"set comm remote GET$/boardshortmessages.json\r\n");
...
//    sprintf(OutString,"join # 1\r\n");  // NOT GOOD SINCE THIS CAN CHANGE EVERY MINUTE!!!
  ...
  if (MessageCheck)
  {
    ...
    sprintf(OutString, "                \n                ");  // 41, 59
    temp2 = WiFlyMsgLen %10;
    temp3 = (char) (WiFlyMsgLen - temp2) /10;
  ...
  Delayms(2000);
    ...
    if (WiFlyMsgLen <= 15)
    {
      for (jloop = 0; jloop<=WiFlyMsgLen; jloop++)
      {  OutString[jloop] = WiFlyMsg[jloop]; }

    }
    else if (WiFlyMsgLen >= 31 )
    {
...
//      OutString[17] = 'n';
      ...
      for (jloop = 16; jloop<=WiFlyMsgLen; jloop++)
      {  OutString[jloop+1] = WiFlyMsg[jloop]; }
    }
    LCD_only_out(OutString);
      ...
      if ( (WiFlyMsg[jloop] >=0x30) && (WiFlyMsg[jloop] <=0x39) )
        {
          ...
          if (jloop %2 == 0)
            ...
            { NewChannel[(jloop-1)/2 - 4] = 10*temp1 + WiFlyMsg[jloop] - 0x30; }
        }
      else
        {
          MsgGood = 0;
    ...
    for (jloop = 0; jloop <=6; jloop++)
      {
        ...
        if (Channel[jloop] == 0)  // ignore this channel
        {
            ...
            ChanTime = 0;
            sprintf(OutString, "\r\nCharSel=       \r\n");
            OutString[12] = ChanSel + 0x30;
            putsConsole(OutString);
        }
        else
        {
           if (ChanTime >= Channel[jloop])
             {
               LED_D3 = 0;
               LED_D4 = 0;
               LED_D5 = 0;
