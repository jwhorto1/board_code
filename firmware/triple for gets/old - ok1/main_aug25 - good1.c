...
//    putsWiFly(OutString);
  Delayms(200);
    sprintf(OutString, "Aug25 8:08PM    \ngets only       ");  // columns 41, 59
    putsConsole(OutString);
  Delayms(200);
    putsLCD(OutString);
  Delayms(2000);
...
//    WiFlyMsg_ptr = &WiFlyMsg[0];
     ...
     if ((iloop >= 0) && (iloop <= 1))   sprintf(OutString,"ftp get aug19_00.txt\r\n");
     if ((iloop >= 2) && (iloop <= 3))   sprintf(OutString,"ftp get aug19_01.txt\r\n");
     if ((iloop >= 4) && (iloop <= 5))   sprintf(OutString,"ftp get aug19_02.txt\r\n");
     if ((iloop >= 6) && (iloop <= 7))   sprintf(OutString,"ftp get aug19_03.txt\r\n");
     if ((iloop >= 8) && (iloop <= 9))   sprintf(OutString,"ftp get aug19_04.txt\r\n");
     if ((iloop >= 10) && (iloop <= 11)) sprintf(OutString,"ftp get aug19_05.txt\r\n");
     if ((iloop >= 12) && (iloop <= 13)) sprintf(OutString,"ftp get aug19_06.txt\r\n");
  ...
  Delayms(200);
    ...
    sprintf(OutString,"     char cnt:  ");
    putsConsole(OutString);
    length = WiFlyMsgLen %10;
    ones_char = length + 0x30;
    length = (WiFlyMsgLen - length)/10;
    tens_char = length + 0x30;
    putConsole(tens_char);
    putConsole(ones_char);
    sprintf(OutString,"\r\n\r\n");
    putsConsole(OutString);
