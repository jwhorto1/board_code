...
//    putsWiFly(OutString);
  Delayms(200);
    sprintf(OutString, "Aug27 8:38PM    \ngets only       ");  // columns 41, 59
    putsConsole(OutString);
  Delayms(200);
    putsLCD(OutString);
  Delayms(2000);
...
//while (1);
    ...
    {
    sprintf(OutString,"ftp get aaad26_00.txt\r\n");
    temp1 = iloop %10;
    ones_char = temp1 + 0x30;
    temp2 = (iloop - temp1)/10;
    tens_char = temp2 + 0x30;
    OutString[15] = tens_char;
    OutString[16] = ones_char;
    putsConsole(OutString);
    putsWiFly(OutString);
  Delayms(2000);
    sprintf(OutString,"\r\n and I saw this:   ");
  ...
  Delayms(200);
    ...
    putsConsole(OutString);
    temp1 = WiFlyMsgLen %10;
    ones_char = temp1 + 0x30;
    temp2 = (WiFlyMsgLen - temp1)/10;
    tens_char = temp2 + 0x30;
...
//    putsConsole(OutString);
    ...
    for ( jloop = 0; jloop <= WiFlyMsgLen; jloop ++)
        {
         temp4 = WiFlyMsg[jloop] - 0x30;
         if (temp4 < 0) temp4 = 0;
         if (temp4 > 10) temp4 = 10;
         sum[iloop] = sum[iloop] + (unsigned int)temp4;
        }
    temp1 = sum[iloop] %10;
    ones_char = temp1 + 0x30;
    temp2 = ((sum[iloop] - temp1)/10 ) %10;
    tens_char = temp2 + 0x30;
    temp3 = (sum[iloop] - 10*temp2 - temp1)/100;
    huns_char = temp3 + 0x30;
    sprintf(OutString,"     sum:  ");
    putsConsole(OutString);
    putConsole(huns_char);
    putConsole(tens_char);
    putConsole(ones_char);
    sprintf(OutString,"\r\n\r\n");
    putsConsole(OutString);
