...
// Message Trap
  if (AssociatedCheck  &&  (MessageCheck <2))  // Associated successfully found, now we are in iloop
  {
      ...
      else if ( (MessageCnt == 1) && (U1RXREG == 'o') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        }
      else if ( (MessageCnt == 2) && (U1RXREG == 'n') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        MessageIncoming = 1;
        ...
        if (U1RXREG == 'r')
            ...
            *WiFlyMsg_ptr = U1RXREG;
            if (WiFlyMsg_ptr >= &WiFlyMsg[0] + 127) {WiFlyMsg_ptr = &WiFlyMsg[0];}
            WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
            if (WiFlyMsgLen > 30) MessageIncoming = 0; //Associated! is 11 characters
            }
        }
