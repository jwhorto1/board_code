...
char AssociatedIncoming; //JKW
char AssociatedCheck; //JKW
int  MessageCnt;
char MessageIncoming;
char MessageCheck;
...
// Associated! Trap
  if (!AssociatedCheck && (U1RXREG == 'A') )
  {
      AssociatedIncoming = 1;
      WiFlyMsgLen = 0;
      WiFlyMsg_ptr = &WiFlyMsg[0];
      MessageCnt = 40;
  }

  if (!AssociatedCheck && AssociatedIncoming)
  {
      MessageCnt = 40;
      if (U1RXREG == '!')
      {
      AssociatedIncoming = 0;
      WiFlyMsg_ptr++;
      ...
      *WiFlyMsg_ptr = 0x00;
      WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
         if (WiFlyMsgLen < 20) //Associated! is 11 characters
          {
            AssociatedCheck = 1;
            MessageIncoming = 0;
            MessageCheck = 2;  // leaves with a signature 2, can't proceed until iloop in main
        }
      }
      else
      {
      WiFlyMsg_ptr++;
      *WiFlyMsg_ptr = U1RXREG;  // First time thru gets 'A'
      if (WiFlyMsg_ptr >= &WiFlyMsg[0] + 127) {WiFlyMsg_ptr = &WiFlyMsg[0];}
      WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
      if (WiFlyMsgLen > 11) AssociatedIncoming = 0; //Associated! is 11 characters
      }
  }


// Message Trap
  if (AssociatedCheck  &&  (MessageCheck <2))  // Associated successfully found, now we are in iloop
  {
      if (U1RXREG == 'a')
        {
        WiFlyMsgLen = 0;
      ...
      else if ( (MessageCnt == 1) && (U1RXREG == 's') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        //LED_D4 = 1;
        }
      else if ( (MessageCnt == 2) && (U1RXREG == 'c') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        //LED_D5 = 1;
        }
      else if ( (MessageCnt == 3) && (U1RXREG == 'i') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        //LED_D6 = 1;
        }
      else if ( (MessageCnt == 4) && (U1RXREG == 'i') )  // The immediate next one
        {
        WiFlyMsg_ptr++;
        *WiFlyMsg_ptr = U1RXREG;
        MessageIncoming = 1;
        ...
        {
         // *** LATER GET RID OF INDICATORS.H ************
        //LED_D3 = 0;

        if (U1RXREG == 0x22)  // 0x22 is '"'
            {
            WiFlyMsg_ptr++;
            *WiFlyMsg_ptr = U1RXREG;
            WiFlyMsg_ptr++;
            ...
            *WiFlyMsg_ptr = 0x00;
            WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
            //if (WiFlyMsgLen < 20) MessageCheck = 1; //Message size test
            //else MessageCheck = 0;
            }
        else
            {
            ...
            *WiFlyMsg_ptr = U1RXREG;
            if (WiFlyMsg_ptr >= &WiFlyMsg[0] + 127) {WiFlyMsg_ptr = &WiFlyMsg[0];}
            WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
            if (WiFlyMsgLen > 80) MessageIncoming = 0; //Associated! is 11 characters
            }
        }
    else    // Message didn't pass muster
    {MessageCnt = 40;}
  }
