  ...
  if (AssociatedIncoming)
  {
      ...
      *WiFlyMsg_ptr = 0x00;
      ...
      WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
         if (WiFlyMsgLen < 20)
          {
              AssociatedCheck = 1; //Associated! is 11 characters
            MessageTest1 = 0;
            MessageTest2 = 0;
            MessageIncoming = 0;
            MessageCheck = 0;
...
/* *******
// Message Trap
  if ( !AssociatedIncoming && (U1RXREG == 'W') )
  {
      MessageTest1 = 1;
      WiFlyMsgLen = 0;
      WiFlyMsg_ptr = &WiFlyMsg[0];
  ...
  if (MessageIncoming)
  {
      ...
      *WiFlyMsg_ptr = 0x00;
      WiFlyMsg_ptr++;
      WiFlyMsgLen = WiFlyMsg_ptr - &WiFlyMsg[0];
      if (WiFlyMsgLen < 20) AssociatedCheck = 1; //Associated! is 11 characters
      }
      else
      {
      *WiFlyMsg_ptr = U1RXREG;
